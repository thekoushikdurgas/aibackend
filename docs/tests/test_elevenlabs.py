...
class TestElevenLabsTextToSpeechService:
    ...
    async def test_list_voices(self, mock_voices_response):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            mock_client.return_value.__aenter__.return_value.get = AsyncMock(
                return_value=mock_voices_response
            )
    ...
    @pytest.mark.asyncio
    async def test_validate_text_length_valid(
        self, mock_models_response, mock_voices_response
    ):
        """Test text length validation with valid text"""
        with patch("httpx.AsyncClient") as mock_client:
            mock_get = AsyncMock(
                side_effect=[mock_models_response, mock_voices_response]
            )
            mock_client.return_value.__aenter__.return_value.get = mock_get

            service = ElevenLabsTextToSpeechService(api_key="test-key")
            result = await service.validate_text_length(
                "Short text", "eleven_multilingual_v2"
            )
    ...
    async def test_generate_tts(
        self, mock_tts_response, mock_voices_response, mock_models_response
    ):
        """Test TTS generation"""
        with patch("httpx.AsyncClient") as mock_client:
            mock_get = AsyncMock(
                side_effect=[mock_voices_response, mock_models_response]
            )
    ...
    @pytest.mark.asyncio
    async def test_generate_tts_with_voice_settings(
        self, mock_tts_response, mock_voices_response, mock_models_response
    ):
    ...
    async def test_generate_tts_invalid_voice(self, mock_voices_response):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with pytest.raises(Exception) as exc_info:
                await service.generate(text="Test", voice_id="invalid_voice_id")
    ...
    @pytest.mark.asyncio
    async def test_generate_tts_api_error(
        self, mock_voices_response, mock_models_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_tts_rate_limit(
        self, mock_voices_response, mock_models_response
    ):
    ...
    async def test_generate_stream(self, mock_voices_response, mock_models_response):
        ...
        stream_response.aiter_bytes = AsyncMock(
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            async for chunk in service.generate_stream(
                text="Test streaming", voice_id="pMsXgVXv3BLzUgSXRplE"
            ):
                chunks.append(chunk)
