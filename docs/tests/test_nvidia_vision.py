...
@pytest.mark.asyncio
async def test_nvidia_vision_analyze_with_image_url(
    nvidia_vision_service, mock_httpx_response
):
...
@pytest.mark.asyncio
async def test_nvidia_vision_analyze_with_base64(
    nvidia_vision_service, mock_httpx_response
):
    ...
    with patch("app.services.nvidia.client.httpx.AsyncClient") as mock_client:
        ...
        result = await nvidia_vision_service.analyze(
            prompt="Describe this image", image=base64_image
        )
...
@pytest.mark.asyncio
async def test_nvidia_vision_analyze_multimodal(
    nvidia_vision_service, mock_httpx_response
):
    ...
    with patch("app.services.nvidia.client.httpx.AsyncClient") as mock_client:
        ...
        result = await nvidia_vision_service.analyze_multimodal(
            prompt="Compare these images", images=images
        )
...
@pytest.mark.asyncio
async def test_nvidia_vision_analyze_video_frames(
    nvidia_vision_service, mock_httpx_response
):
    ...
    with patch("app.services.nvidia.client.httpx.AsyncClient") as mock_client:
        ...
        result = await nvidia_vision_service.analyze_video_frames(
            prompt="What happens in this video?", frames=frames
        )
...
async def test_nvidia_vision_get_model_info(nvidia_vision_service):
    ...
    info = await nvidia_vision_service.get_model_info(
