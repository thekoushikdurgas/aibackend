...
def test_document_chunking_recursive():
    ...
    chunker_instance = DocumentChunker(
        chunk_size=100, chunk_overlap=20, strategy="recursive"
    )
...
def test_document_chunking_sliding_window():
    ...
    chunker_instance = DocumentChunker(
        chunk_size=100, chunk_overlap=20, strategy="sliding"
    )
...
def test_chunk_multiple_documents():
    ...
    assert "doc1" in sources or any(
        "doc1" in str(chunk.get("id", "")) for chunk in chunks
    )
    assert "doc2" in sources or any(
        "doc2" in str(chunk.get("id", "")) for chunk in chunks
    )
...
async def test_rag_pipeline_query():
    ...
    try:
        result = await rag_pipeline.query(query="test query", top_k=3)
