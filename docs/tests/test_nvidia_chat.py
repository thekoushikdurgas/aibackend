...
async def test_nvidia_chat_generate(nvidia_chat_service, mock_httpx_response):
    ...
    with patch("app.services.nvidia.client.httpx.AsyncClient") as mock_client:
        ...
        response = await nvidia_chat_service.generate(prompt="Hello", config=config)
...
async def test_nvidia_chat_get_model_info(nvidia_chat_service):
    ...
    info = await nvidia_chat_service.get_model_info(
...
@pytest.mark.asyncio
async def test_nvidia_chat_with_conversation_history(
    nvidia_chat_service, mock_httpx_response
):
    ...
    with patch("app.services.nvidia.client.httpx.AsyncClient") as mock_client:
        ...
        response = await nvidia_chat_service.generate(
            prompt="What did I say?", conversation_history=history
        )
...
async def test_nvidia_chat_reasoning_model(nvidia_chat_service, mock_httpx_response):
    ...
    with patch("app.services.nvidia.client.httpx.AsyncClient") as mock_client:
        mock_httpx_response.json.return_value["choices"][0]["message"][
            "reasoning_content"
        ] = "Reasoning steps"
        mock_client.return_value.__aenter__.return_value.post = AsyncMock(
            return_value=mock_httpx_response
        ...
        response = await nvidia_chat_service.generate(
            prompt="Solve this", config=config
        )
