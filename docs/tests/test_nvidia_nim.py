...
def nvidia_nim_service():
    ...
    return NVIDIANIMService(api_key="test_key", nim_base_url="https://nim.example.com/v1")


@pytest.mark.asyncio
async def test_nvidia_nim_service_initialization():
    """Test NVIDIA NIM service initialization"""
    service = NVIDIANIMService(api_key="test_key", nim_base_url="https://nim.example.com/v1")
...
async def test_nvidia_nim_health_check(nvidia_nim_service, mock_httpx_response):
    ...
    with patch('app.services.nvidia.client.httpx.AsyncClient') as mock_client:
        mock_client.return_value.__aenter__.return_value.get = AsyncMock(return_value=mock_httpx_response)
...
async def test_nvidia_nim_infer(nvidia_nim_service):
    ...
    with patch('app.services.nvidia.client.httpx.AsyncClient') as mock_client:
        ...
        result = await nvidia_nim_service.infer(
            model_id="model1",
            messages=messages
        )
