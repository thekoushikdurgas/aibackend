...
class TestModelSelector:
    ...
    async def test_select_council_models_no_providers(self):
        ...
        with patch(
            "app.services.council.model_selector.LLMProviderFactory"
        ) as mock_factory:
    ...
    async def test_select_chairman_model(self):
        ...
        with patch(
            ...
            with patch.object(
                ModelSelector, "_check_provider_health", new_callable=AsyncMock
            ) as mock_health:
...
class TestCouncilOrchestrator:
    ...
    def sample_page_data(self):
        ...
        return PageData(
            url="https://example.com", title="Example Page", domain="example.com"
        )
    ...
    async def test_calculate_aggregate_rankings(self):
        ...
        label_to_model = {"Response A": "provider_a", "Response B": "provider_b"}

        result = orchestrator.calculate_aggregate_rankings(
...
class TestCouncilAgent:
    ...
    def sample_page_data(self):
        """Sample page data"""
        return PageData(url="https://example.com", title="Test Page")
    ...
    async def test_council_agent_analyze_no_models(self):
        ...
        with patch(
            "app.services.council.orchestrator.run_full_council"
        ) as mock_council:
            mock_council.return_value = (
                [],
                [],
                {"model": "error", "response": "No models available"},
                {},
            )
...
async def test_integration_council_flow():
    ...
    with patch("app.services.council.orchestrator.CouncilOrchestrator") as mock_orch:
        ...
        mock_instance.run_full_council = AsyncMock(
            return_value=(
