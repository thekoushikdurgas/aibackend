...
from app.services.openrouter import OpenRouterModelRegistry, OpenRouterEmbeddingService, get_monitor
...
class TestOpenRouterModelRegistry:
    ...
    async def test_auto_route(self):
        ...
        result = await registry.auto_route(
...
class TestOpenRouterProviderAdvanced:
    ...
    async def test_generate_with_cost_tracking(self, mock_openrouter_provider):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            # Mock get_model_info
            with patch.object(mock_openrouter_provider, "get_model_info", new_callable=AsyncMock) as mock_info:
    ...
    async def test_generate_with_retry(self, mock_openrouter_provider):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with patch("asyncio.sleep", new_callable=AsyncMock):
                with patch.object(mock_openrouter_provider, "get_model_info", new_callable=AsyncMock):
                    config = LLMConfig(model="openai/gpt-4o")
                    response = await mock_openrouter_provider.generate(
                        prompt="Test",
                        config=config
