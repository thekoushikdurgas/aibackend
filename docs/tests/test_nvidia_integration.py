...
def mock_nvidia_services():
    ...
    with patch("app.services.nvidia.chat.NVIDIAChatService") as mock_chat, patch(
        "app.services.nvidia.embeddings.NVIDIAEmbeddingService"
    ) as mock_emb, patch(
        "app.services.nvidia.vision.NVIDIAVisionService"
    ) as mock_vision, patch(
        "app.services.nvidia.nim.NVIDIANIMService"
    ) as mock_nim:

        # Setup mock chat service
        mock_chat_instance = MagicMock()
        mock_chat_instance.generate = AsyncMock(
        ...
        mock_chat_instance.list_models = AsyncMock(
            return_value=["nvidia/llama-3.3-nemotron-super-49b-v1"]
        )
        mock_chat_instance.get_model_info = AsyncMock(
            return_value={
        ...
        mock_emb_instance.embed = AsyncMock(
            return_value={
        ...
        mock_emb_instance.list_models = AsyncMock(
            return_value=["nvidia/nv-embedqa-e5-v5"]
        )
        mock_emb.return_value = mock_emb_instance

        # Setup mock vision service
        mock_vision_instance = MagicMock()
        mock_vision_instance.analyze = AsyncMock(
            return_value={
                "text": "This is a cat",
        ...
        mock_vision_instance.list_models = AsyncMock(
            return_value=["meta/llama-3.2-90b-vision-instruct"]
        )
...
async def test_nvidia_embeddings_endpoint(mock_nvidia_services):
    ...
    response = client.post(
        ...
        json={"input": "Hello world", "model": "nvidia/nv-embedqa-e5-v5"},
    )
...
async def test_nvidia_integrated_chat_route(mock_nvidia_services):
    ...
    with patch(
        "app.services.llm.factory.LLMProviderFactory.get_provider"
...
async def test_nvidia_integrated_vision_route(mock_nvidia_services):
    ...
    response = client.post(
        ...
        json={"prompt": "What is this?", "image_url": "https://example.com/image.jpg"},
    )
