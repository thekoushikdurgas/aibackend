        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                if "analysis" in data:
                    ...
                    print(
                        f"[CHECK] response.analysis keys: {list(analysis.keys()) if isinstance(analysis, dict) else 'N/A'}"
                    )
                    print(
                        "[CHECK] response.analysis.error exists:",
                        "error" in analysis if isinstance(analysis, dict) else "N/A",
                    )
                    print(
                        "[CHECK] response.analysis.final_response exists:",
                        (
                            "final_response" in analysis
                ...
                if "summary" in data:
                    print(
                        f"[CHECK] response.summary length: {len(data['summary'])} chars"
                    )
                    print(
                        f"[CHECK] response.summary preview: {data['summary'][:200]}..."
                    )
