...
Configure providers in **`ai.backend/.env`** (see `.env.example`). The catalog of all Postman-backed providers is in [`config/provider_manifest.json`](../config/provider_manifest.json).

| Env pattern           | Example                              |
| --------------------- | ------------------------------------ |
| `{PROVIDER}_API_KEY`  | `OPENAI_API_KEY`, `DEEPSEEK_API_KEY` |
| `{PROVIDER}_BASE_URL` | `MISTRAL_BASE_URL`                   |
| `{PROVIDER}_MODEL`    | `GROQ_MODEL`                         |

OpenAI-compatible hosts use `app/services/llm/openai_compat.py`. Native adapters: Gemini, Cohere, AI21, Ollama, HuggingFace, NVIDIA, Bedrock, Vertex, watsonx, DashScope.

### Common Configuration Fields

All chat providers support:

- API key (env `*_API_KEY`)
- Base URL (`*_BASE_URL`) — defaults from manifest / Postman collection
- Default model (`*_MODEL`)
