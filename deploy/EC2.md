...
| `ModuleNotFoundError` in backend logs | Missing pip package in `requirements.txt`. Rebuild: `docker compose --env-file .env -f compose.yaml build --no-cache backend && … up -d --force-recreate backend`. |
