...
| `Restarting (1)` / `Could not connect to 127.0.0.1:8000` | Backend **crash-loop** (often OOM or multi-worker Chroma). `git pull`, then `bash deploy/remote-deploy.sh` (recreates backend with 1 worker, Ollama embeddings, deferred Chroma init). `docker compose --env-file .env -f compose.yaml logs backend --tail 100` |
