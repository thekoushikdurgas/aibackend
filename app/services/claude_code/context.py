"""
Workspace context for Claude Code harness.
"""
...
def build_port_context(base: Path | None = None) -> PortContext:
    """Build context; `base` defaults to the `claude_code` package directory."""
    root = base or Path(__file__).resolve().parent
    source_root = root
    tests_root = root.parent.parent / "tests"
    assets_root = root / "reference_data"
    archive_root = (
        root.parent.parent.parent / "archive" / "claude_code_ts_snapshot" / "src"
    )
    return PortContext(
        ...
        test_file_count=(
            sum(1 for path in tests_root.rglob("*.py") if path.is_file())
            if tests_root.exists()
            else 0
        ),
        asset_file_count=(
            sum(1 for path in assets_root.rglob("*") if path.is_file())
