"""
Mirrored tool inventory from reference_data (ported from claw-code-parity-main).
"""
...
def get_tools(
    ...
    if simple_mode:
        tools = [
            m for m in tools if m.name in {"BashTool", "FileReadTool", "FileEditTool"}
        ]
    if not include_mcp:
        tools = [
            m
            for m in tools
            if "mcp" not in m.name.lower() and "mcp" not in m.source_hint.lower()
        ]
