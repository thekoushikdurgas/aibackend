...
from .groq_models import GroqModelSelector
...
class GroqProvider(BaseLLMProvider):
    ...
    Known for ultra-fast inference with low latency.
    """
    ...
    def _build_messages(
        ...
        """
        Build messages array for Groq chat completions API.
    ...
    async def list_models(self) -> List[str]:
        ...
        return GroqModelSelector.list_models_by_category()
    ...
    def select_optimal_model(
        ...
        Args:
            task_type: Type of task - "speed", "reasoning", "vision", "coding", "long_context"
        ...
        """
        return GroqModelSelector.select_model(task_type, complexity, requirements)
    ...
    async def generate_with_vision(
        ...
        if not config.model:
            config.model = self.select_optimal_model("vision", "medium")
        ...
        try:
            ...
            if hasattr(e, "response") and e.response is not None:
                try:
                    error_data = e.response.json()
                    error_msg = error_data.get("error", {}).get("message", str(e))
                    raise Exception(f"Groq vision API error: {error_msg}")
                except (ValueError, AttributeError, KeyError):
                    pass
