...
async def handle_chat_providers(
    ...
    from app.services.llm.manifest_compat import enrich_provider_info
    ...
    for name in LLMProviderFactory.list_providers():
        try:
            ...
            providers.append(enrich_provider_info(name, row))
        except Exception as e:
            row = {"name": name, "status": "error", "error": str(e), "models": []}
            providers.append(enrich_provider_info(name, row))
