...
GEMMA_CHECKPOINTS = [
    {"id": "GEMMA2_2B_IT", "path": "gs://gemma-data/checkpoints/gemma2-2b-it", "version": "Gemma 2", "size": "2B", "type": "Instruct"},
    {"id": "GEMMA3_270M_IT", "path": "gs://gemma-data/checkpoints/gemma3-270m-it", "version": "Gemma 3", "size": "270M", "type": "Instruct"},
    {"id": "GEMMA3_4B_IT", "path": "gs://gemma-data/checkpoints/gemma3-4b-it", "version": "Gemma 3", "size": "4B", "type": "Instruct"},
    {"id": "GEMMA3_12B_IT", "path": "gs://gemma-data/checkpoints/gemma3-12b-it", "version": "Gemma 3", "size": "12B", "type": "Instruct"},
    {"id": "GEMMA4_E2B_IT", "path": "gs://gemma-data/checkpoints/gemma4-e2b-it", "version": "Gemma 4", "size": "E2B", "type": "Instruct"},
    {"id": "GEMMA4_E4B_IT", "path": "gs://gemma-data/checkpoints/gemma4-e4b-it", "version": "Gemma 4", "size": "E4B", "type": "Instruct"},
]
...
async def handle_gemma_chat_completions(
    ...
    model = params.get("model", settings.gemma_model)
    stream = params.get("stream", False)
    temperature = float(params.get("temperature", 0.7))
    mode = params.get("mode", settings.gemma_mode).lower()
    ...
    if mode == "local":
        try:
            ...
            logger.warning("JAX or gemma package not found, falling back to simulated mode")
            mode = "simulated"
    ...
    if mode == "api":
        try:
            ...
            config = LLMConfig(model=model or "google/gemma-7b-it", temperature=temperature)
            if stream:
...
async def _stream_ollama(provider: Any, message: str, config: LLMConfig) -> AsyncGenerator[Dict[str, Any], None]:
...
async def _stream_api(provider: Any, message: str, config: LLMConfig) -> AsyncGenerator[Dict[str, Any], None]:
...
def _generate_simulated_gemma_response(message: str, model: str, temperature: float) -> str:
    ...
    if "hello" in msg_lower or "hi" in msg_lower:
        return (
            f"Hello! I am Google Gemma ({model}), running in simulated local mode. How can I assist you with your JAX/Flax developments today?"
        )
...
async def handle_gemma_checkpoints_list(
    ...
    for ckpt in GEMMA_CHECKPOINTS:
        c = ckpt.copy()
        c["status"] = "available" if has_local and ckpt["id"] == "GEMMA3_4B_IT" else "download_required"
        result.append(c)
...
async def handle_gemma_checkpoints_load(
    ...
    checkpoint = next((c for c in GEMMA_CHECKPOINTS if c["id"] == checkpoint_id), GEMMA_CHECKPOINTS[2])
...
async def handle_gemma_finetune_run(
    ...
    adapter_dir = Path(settings.storage_root) / settings.storage_bucket_uploads / "gemma_adapters"
    ...
    try:
        ...
        with open(adapter_file, "w") as f:
            f.write(f"DUMMY GEMMA LORA ADAPTER WEIGHTS\nDataset: {dataset}\nEpochs: {epochs}\nRank: {rank}\nAlpha: {alpha}\nLoss: {current_loss:.4f}\n")
